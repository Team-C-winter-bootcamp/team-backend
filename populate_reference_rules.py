...
from precedents.models import ReferenceRule
...
def main():
    # Clear existing rules and reset PK sequence
    with connection.cursor() as cursor:
        cursor.execute("TRUNCATE TABLE precedents_referencerule RESTART IDENTITY CASCADE;")
    print("Cleared all existing reference rules and reset primary key sequence.")
                ...
                if reference_info:
                    ...
                    if reference_rules_str:
                        ...
                        for law_type, article_no in parsed_rules:
                            obj, created = ReferenceRule.objects.get_or_create(
                                law_type=law_type,
                                article_no=article_no,
                                defaults={'is_deleted': False}
                            )
                            if created:
                                print(f"Created: {law_type} 제{article_no}조")
